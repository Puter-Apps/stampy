...
interface AddSiteDialogProps {
  isOpen: boolean;
  onAddSite: (name: string, sitemapUrl: string) => Promise<void>;
  onCancel: () => void;
}

function AddSiteDialog({ isOpen, onAddSite, onCancel }: AddSiteDialogProps) {
...
interface SidebarProps {
  onUpdateDocument: (site: Site) => void;
}

export default function Sidebar({ onUpdateDocument }: SidebarProps) {
  ...
  const saveWebsites = async (websitesList: Site[]) => {
    try {
  ...
  const parseSitemapUrls = (xmlContent: string): string[] => {
    const parser = new DOMParser();
  ...
  const handleAddSite = async (name: string, sitemapUrl: string) => {
    let sitemapContent: string;
    let urls: string[];
    let htmlContents: (string | null)[];
    let documents: { id: string; title: string; text: string }[];
    let hostname: string;
